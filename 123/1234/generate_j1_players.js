...
function translateToKorean(name, position) {
  ...
  const japaneseSurnames = {
    ...
    'Tanimura': '타니무라', 'Imai': '이마이', 'Myogan': '묘간',
    'Nago': '나고', 'Akino': '아키노', 'Brown': '브라운', 'Kim': '김',
    'Furuyama': '후루야마', 'Nishio': '니시오', 'Yoshino': '요시노',
    'Ienaga': '이에나가', 'Fukuda': '후쿠다', 'Tawiah': '타위아',
    'Lavi': '라비', 'Narasaka': '나라사카', 'Miya': '미야',
    'Sekine': '세키네', 'Niekawa': '니에카와', 'Tono': '토노', 'Amano': '아마노',
    'Nakashima': '나카시마', 'Matsuyama': '마츠야마',
  };
  ...
  const portugueseNames = {
    ...
    'Miguel': '미겔', 'Silveira': '실베이라', 'Lucas': '루카스', 'Juan': '후안', 'Alano': '알라노',
    'William': '윌리엄',
  };
  ...
  const englishNames = {
    ...
    'Brodersen': '브로더센', 'Schmidt': '슈미트', 'Brown': '브라운',
  };
  ...
  const fullNameMappings = {
    ...
    'Toya Myogan': '토야 묘간',
    'Patrick William': '파트릭 윌리엄',
    'Tomoki Imai': '토모키 이마이',
    'Kaung Zan Mara': '카웅 잔 마라',
    'Ghaith Mansour Hamid Al Mhairat': '가이스 만수르 하미드 알 므하이라트',
  ...
  // 전체 이름이 매핑되어 있는지 먼저 확인
  if (fullNameMappings[name]) return fullNameMappings[name];
  ...
  // 성 기반 매핑 (일본어 이름은 마지막이 성)
  if (parts.length > 0) {
    const surname = parts[parts.length - 1]; // 마지막 단어가 성

    if (japaneseSurnames[surname]) {
      const koreanSurname = japaneseSurnames[surname];
      if (parts.length > 1) {
        const initials = parts.slice(0, -1).join(' '); // 이니셜 부분
        return `${initials} ${koreanSurname}`;
      }
    ...
    if (portugueseNames[surname]) {
      ...
      if (parts.length > 1) {
        ...
        if (portugueseNames[givenName]) {
          return `${portugueseNames[givenName]} ${koreanSurname}`;
        }
