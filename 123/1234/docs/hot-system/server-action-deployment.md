# HOT 알림 시스템 배포 가이드 (Server Actions)

HOT 게시글 알림 시스템은 **Next.js Server Actions**와 **Vercel Cron Jobs**를 사용합니다.
...
| 항목 | Edge Functions (이전) | Server Actions (현재) |
|------|---------------------|---------------------|
| 배포 | Supabase CLI 필요 | Vercel 자동 배포 |
| 크론 설정 | pg_cron 수동 설정 | vercel.json 한 줄 |
| 코드 위치 | supabase/functions/ | src/domains/notifications/ |
| 테스트 | Supabase 환경 필요 | 로컬에서 바로 가능 |
| 유지보수 | 별도 관리 | 프로젝트 코드와 통합 |

---

**작성일**: 2025-12-03
**최종 업데이트**: 2025-12-03
**버전**: 2.0.0 (Server Actions)
