...
* **원인**: `posts` 테이블의 최근 글 100개를 가져오지만, 내부적으로 쿠키 세션을 읽어오는 `getSupabaseServer` 및 `getSeoSettings` API 의존성이 존재하여 Next.js가 해당 Route를 강제로 Dynamic Rendering으로 빌드하고 있습니다.
* **리스크**: RSS 봇이 수시로 피드를 긁어갈 때마다 매번 CDN 캐싱 없이 Supabase DB에 실시간 Direct Hit를 수행하여 불필요한 Database Egress 트래픽을 낭비합니다.
* **상세 분석 및 개선안**: [supabase_rss_egress_optimization.md](file:///home/kim/web2/.gemini/here/supabase_rss_egress_optimization.md) 문서를 참고하여 `getSupabaseClientNoCookies` 클라이언트 대체 및 15분 간격의 `revalidate = 900` 정적 캐싱(ISR)을 적용할 수 있습니다.
