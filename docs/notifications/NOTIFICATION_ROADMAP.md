...
**크론잡 설정**: ✅ **매 시간 정각 자동 실행 중 (Production)**
- **배포일**: 2025-12-03
- **실행 환경**: Supabase Edge Function + pg_cron
- **스케줄**: `0 * * * *` (매시간 0분)
- **이전 방식**: Vercel Cron (일 1회 제한) → Supabase (시간당 실행)

**관련 문서**:
- [HOT 점수 계산 가이드](../sidebar/HOT_SCORE_GUIDE.md)
- [인기글 시스템 문서](../sidebar/SIDEBAR_POPULAR_POSTS.md)
- [HOT 시스템 현재 아키텍처](../hot-system/edge-function.md) ⭐ **최신**
- [Edge Function README](../../../supabase/functions/check-hot-posts/README.md)
- [배포 가이드](../../../DEPLOY_EDGE_FUNCTION.md)
- [마이그레이션 가이드](../hot-system/supabase-edge-migration.md)
...
| 2025-12-03 | HOT 알림 Supabase 마이그레이션 | ✅ 완료 | Vercel Cron → Supabase Edge Function + pg_cron, 실행 주기 개선 (일 1회 → 시간당 1회) |
