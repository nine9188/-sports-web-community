...
- player도 `player-123` 같은 placeholder slug를 canonical 후보로 쓰지 않는 정책을 명시했다.
- player 상세 자체는 match slug 문제와 직접 같지는 않지만, shell-first URL 정책은 team/match와 동일하게 유지한다.
...
8. slug fallback이 `player-숫자` 같은 placeholder로 굳어지면 canonical 품질이 떨어질 수 있다.
...
- `player`, `player-123`, `123` 같은 placeholder slug는 canonical로 쓰지 않는다.
...
- player slug는 실제 선수명 기반이어야 하며 `player-숫자` placeholder는 canonical 후보에서 제외한다.
...
5. `player-숫자` placeholder가 sitemap/canonical/internal link에 섞이지 않는지 주기적으로 점검한다.
