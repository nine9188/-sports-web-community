...
### 1. `/shop` metadata가 `searchParams`를 받아 query URL을 `noindex`로 분리한다

`generateMetadata({ searchParams })`로 바꾸고, `cat`이나 `page`가 있으면 `noindex`를 넣는다.
canonical은 계속 `/shop`으로 유지한다.

### 2. `page` query 정규화 helper를 둔다

`/shop/[category]`에서 page를 다음 기준으로 정리한다.
- 숫자가 아니면 `1`
- 정수가 아니면 정수로 정리
- 1보다 작으면 `1`

같은 helper를 `/shop` root가 page query를 쓰게 될 경우에도 재사용한다.

### 3. 공개 shop 조회 함수에 요청 단위 cache를 적용한다

우선 다음 조회를 `cache()`로 묶는다.
- `getShopCategories()`
- `getShopCategory(slug)`
- `getCategoryItemsPaginated(categoryIds, page, pageSize)`

이렇게 하면 `generateMetadata()`와 page 본문이 같은 category를 조회할 때 중복 DB 호출을 줄일 수 있다.
구매 액션인 `purchaseItem()`은 캐시 대상이 아니다.

### 4. `/shop` root의 현재 카테고리 fetch 범위를 줄인다

root page도 `cat`과 `page`를 서버에서 읽고, 현재 탭/카테고리에 필요한 아이템만 가져오도록 바꾼다.
전체 500개 선조회는 제거하거나 fallback 범위로 낮춘다.

탭/카테고리 이동은 URL 기반으로 유지한다.
상태 URL은 noindex로 묶고, canonical은 `/shop` 또는 `/shop/[category]` 기본 URL로 둔다.

### 5. shop 목록을 server shell + purchase client island로 나눈다

목록 렌더링은 Server Component로 분리한다.
각 카드의 구매 버튼과 구매 모달만 client island로 남긴다.

최소 수정 기준:
- `CategoryFilter`에서 data fetch와 목록 계산 책임을 줄인다.
- 서버에서 받은 `items`, `activeCategory`, `page`, `totalPages`를 렌더링한다.
- 구매 상태 변경은 기존 `PurchaseModal` / `useShopItems` 계열을 작은 island로 유지한다.

### 6. 이모티콘 탭의 초기 데이터도 서버에서 준비한다

`/shop?cat=<emoticonCategoryId>` 또는 이모티콘 category route에서는 서버에서 `getEmoticonShopData()` 또는 공개 팩 목록을 준비한다.
client에서는 검색, 모달, 구매만 처리한다.

사용자 포인트와 보유 팩은 page에서 이미 조회한 값과 중복되지 않게 한 경로로 통합한다.

### 7. page-level broad catch를 줄이고 `error.tsx`를 사용한다

조회 실패를 빈 상점 UI나 200 오류 화면으로 숨기지 않는다.
복구 가능한 빈 데이터만 UI로 처리하고, 실제 예외는 throw해서 `src/app/(site)/shop/error.tsx`가 받도록 한다.

### 8. shop sitemap과 noindex 정책을 맞춘다

`getShopSitemap()`에서 active item이 없는 category는 제외하거나, page metadata가 index 가능한 category만 sitemap에 넣도록 기준을 맞춘다.

### 9. item image loading 전략을 조정한다

`ItemCard`의 모든 이미지 `loading="eager"`를 제거한다.
필요하다면 첫 화면의 일부 카드만 priority/eager를 적용하고 나머지는 lazy로 둔다.

## 검증 계획

- `npm.cmd run typecheck`
- `npm.cmd run build`
- `/shop` metadata가 indexable인지 확인
- `/shop?cat=...`와 `/shop?page=...` metadata가 `noindex`인지 확인
- `/shop/[category]?page=2`가 정규화된 page로 서버 렌더링되는지 확인
- 이모티콘 탭 직접 진입 시 초기 HTML에 팩 목록이 들어가는지 확인
- sitemap에 noindex shop category가 섞이지 않는지 확인

## 결론

16번 상점 공개 페이지는 수정 전 검토본이다.
현재 route 자체는 Server Component지만, 실제 목록 렌더링과 상태 처리가 큰 client boundary 안에 있고, `/shop` query URL의 검색 정책이 분리되지 않았다.
우선 수정 대상은 `/shop` query noindex, page 정규화, 공개 shop 조회 캐시, root fetch 범위 축소, 목록 server shell 분리, 이모티콘 탭 초기 SSR, sitemap/noindex 정책 정합성이다.
