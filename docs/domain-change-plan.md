...
이 문서는 공개 사이트 도메인을 `4590football.com`에서 `NEW_DOMAIN`으로 바꾸기 위한 실행 계획입니다.

전제:

- URL 경로는 그대로 유지합니다.
- 예: `https://4590football.com/boards/free/123` -> `https://NEW_DOMAIN/boards/free/123`
...
- 현재 CDN 도메인: `cdn.4590football.com`
- 현재 운영 사이트 URL 환경변수: `NEXT_PUBLIC_SITE_URL=https://4590football.com`
- 현재 사이트맵:
  - `https://4590football.com/sitemap.xml`
  - `https://4590football.com/sitemaps/recent-posts.xml`
- 현재 RSS:
  - `https://4590football.com/rss.xml`
- 현재 Cloudflare 네임서버:
...
- `src/app/layout.tsx`
  ...
  - 고객지원 이메일 `support@4590football.com`
  - 인증 태그:
...
- 이메일 제공자
  - `support@4590football.com`에서 `support@NEW_DOMAIN`으로 바꿀 경우 SPF, DKIM, DMARC 설정 필요
