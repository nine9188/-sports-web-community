...
export async function getUserPosts(
  ...
  try {
    ...
    const teamIds = [...new Set((data || []).map((p: any) => p.boards?.team_id).filter(Boolean))] as number[];
    const leagueIds = [...new Set((data || []).map((p: any) => p.boards?.league_id).filter(Boolean))] as number[];
