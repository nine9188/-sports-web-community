...
const VirtualizedPostItem = React.memo(function VirtualizedPostItem({
  ...
  if (isMobile) {
    return (
      <div style={style} className={`py-2 px-3 border-b border-black/5 dark:border-white/10 overflow-hidden ${isCurrentPost ? 'bg-[#EAEAEA] dark:bg-[#333333]' : ''}`}>
        <Link href={href} prefetch={false} className="block">
          <div className="space-y-1">
            <div className="flex items-center gap-1">
              <div className="flex-1 min-w-0 overflow-hidden">
                <span className={`block text-xs truncate ${isCurrentPost ? 'font-medium' : ''} text-gray-900 dark:text-[#F0F0F0] ${post.is_deleted ? 'text-red-500 dark:text-red-400' : post.is_hidden ? 'text-gray-500 dark:text-gray-400' : ''}`}>
...
const PostItem = React.memo(function PostItem({
  ...
  if (isMobile) {
    return (
      <div className={`py-2 px-3 overflow-hidden ${!isLast ? 'border-b border-black/5 dark:border-white/10' : ''} ${isCurrentPost ? 'bg-[#EAEAEA] dark:bg-[#333333]' : ''}`}>
        <Link href={href} prefetch={false} className="block">
          <div className="space-y-1">
            <div className="flex items-center gap-1">
              <div className="flex-1 min-w-0 overflow-hidden">
