        ...
        <div className="px-2 py-3 md:px-4 md:py-4">
          ...
          {goalEvents.length > 0 && (
            <div className="flex flex-col md:flex-row mt-4 md:mt-6 border-t pt-4">
              ...
              <div className="w-full md:w-1/3 relative pl-2 md:px-2 mb-4 md:mb-0 md:text-center">
                ...
                <div className="space-y-1">
                  {goalEvents
                    ...
                    .map((goal: MatchEvent, index: number) => (
                      <div key={`${goal.time.elapsed}-${index}`} className="text-sm text-gray-600 flex items-start md:justify-center">
                        <div className="w-8 flex-shrink-0 text-center mr-1">
                          <span>⚽</span>
                        </div>
                        <div>
              ...
              {/* 스코어 아래 중앙 영역 - 모바일에서는 숨김 */}
              <div className="hidden md:block md:w-1/3">
                {/* 필요시 여기에 경기 상태 또는 시간 관련 정보 표시 가능 */}
              </div>

              {/* 홈/원정팀 구분선 - 모바일에서만 표시 */}
              {goalEvents.filter((event: MatchEvent) => event.team.id === homeTeam?.id).length > 0 && 
               goalEvents.filter((event: MatchEvent) => event.team.id === awayTeam?.id).length > 0 && 
