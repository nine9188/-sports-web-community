...
export default function TeamCard({ team }: TeamCardProps) {
  ...
  return (
    <Link 
      ...
      className={`group block bg-white dark:bg-[#1D1D1D] rounded-lg border transition-colors p-2 lg:p-4 relative ${
        team.isWinner 
          ? 'border-yellow-400 dark:border-yellow-500 hover:bg-yellow-50 dark:hover:bg-yellow-900/20' 
          : 'border-black/7 dark:border-0 hover:bg-[#EAEAEA] dark:hover:bg-[#333333]'
      }`}
    >
      {/* 우승 표시 */}
      {team.isWinner && (
