      ...
      cookies: {
        ...
        set(name, value, options) {
          try {
            ...
            // 서버 컴포넌트에서 쿠키 설정 오류는 무시 (서버 액션에서는 정상 작동)
            if (process.env.NODE_ENV === 'development') {
              console.debug('API 유틸리티에서 쿠키 설정 시도 무시:', name);
            }
          }
        },
        remove(name, options) {
          try {
            ...
            // 서버 컴포넌트에서 쿠키 삭제 오류는 무시 (서버 액션에서는 정상 작동)
            if (process.env.NODE_ENV === 'development') {
              console.debug('API 유틸리티에서 쿠키 삭제 시도 무시:', name);
            }
          }
        },
      },
