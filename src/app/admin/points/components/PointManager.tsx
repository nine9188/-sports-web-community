...
export default function PointManager({
  ...
  const fetchPointHistory = async (userId: string) => {
    try {
      ...
      if (result.success && result.history) {
        // 특정 사용자의 내역만 필터링
        const userHistory = result.history.filter(
  ...
  const handleAdjustPoints = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 성공 메시지 표시
      toast.success(
        `${selectedUser.nickname || "사용자"}님의 포인트가 ${pointAmount > 0 ? "증가" : "차감"}되었습니다.`,
      );
  ...
  return (
    <div className="bg-white dark:bg-[#1D1D1D] rounded-lg border border-black/7 dark:border-white/10 p-6 h-full">
      ...
      {/* 포인트 내역 */}
      <div>
        <h3 className="text-lg font-medium text-gray-900 dark:text-[#F0F0F0] mb-3">
          최근 포인트 내역
        </h3>
