...
export function MatchDataProvider({ 
  ...
  const [homeTeam, setHomeTeam] = useState<{ id: number; name: string; logo: string } | null>(
    initialData.homeTeam || null
  );
  const [awayTeam, setAwayTeam] = useState<{ id: number; name: string; logo: string } | null>(
  ...
  const getTabData = useCallback(async (tab: TabType): Promise<TabData | null> => {
    ...
    try {
      ...
      // 서버 액션 호출
      const fullData = await fetchMatchFullData(matchId, options);
