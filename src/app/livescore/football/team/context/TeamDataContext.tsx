...
import { fetchTeamFullData, TeamResponse } from '@/app/actions/livescore/teams/team';
import { Match } from '@/app/actions/livescore/teams/matches';
import { Standing } from '@/app/actions/livescore/teams/standings';
import { Player, Coach } from '@/app/actions/livescore/teams/squad';
...
export function TeamDataProvider({ 
  ...
  const loadTeamData = useCallback(async (id: string, tab: string = 'overview') => {
    ...
    const needsMatches = requiredData.includes('matchesData') && (!matchesData || !matchesData.data || teamId !== id);
    const needsSquad = requiredData.includes('squadData') && (!squadData || !squadData.data || teamId !== id);
    const needsPlayerStats = requiredData.includes('playerStats') && (!playerStats || !playerStats.data || teamId !== id);
    const needsStandings = requiredData.includes('standingsData') && (!standingsData || !standingsData.data || teamId !== id);
