...
const BoardSelector = React.memo(({ 
  ...
  // boards를 계층형 구조로 변환하는 함수 - 메모이제이션으로 불필요한 계산 방지
  const organizedBoards = useMemo(() => {
  ...
  // 드롭다운 옵션 렌더링 함수
  const renderBoardOptions = useCallback((boards: Board[], level: number = 0) => {
  ...
  }, [selectedId, currentBoardId, handleSelect]);
  ...
  const selectedBoardName = useMemo(() => {
    ...
    return findBoardName(selectedId || '');
  }, [selectedId, organizedBoards]);
    ...
    <div className="w-full relative" ref={dropdownRef}>
      ...
      {showDropdown && (
        <div className="absolute z-10 mt-1 w-full bg-white border border-gray-200 rounded-md shadow-lg max-h-60 overflow-y-auto">
          <div className="py-1">
            {renderBoardOptions(organizedBoards)}
          </div>
        </div>
      )}
    </div>
